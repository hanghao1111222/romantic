...
## 📖 使用教学与交互玩法 (Interactive Guide)

> 💡 完整进阶教程与录屏摄影技巧请见：**[完整使用指南 (TUTORIAL.md)](TUTORIAL.md)**

### 🌹 1. 3D 粒子花朵实验室操作指南
* **360° 旋转视角：** 鼠标在画面中按住左键自由拖动，即可全方位环视花朵绽放细节与花瓣层次；
* **空间焦距缩放：** 滚动鼠标滚轮（Wheel）或按键盘 `+` / `-` 键；
* **键盘快捷键：**
  - **`[V]`** 切换花型（玫瑰束、绣球、独枝玫瑰、郁金香、百合、牡丹、幽谷飞兰、心形花、时空星系）
  - **`[C]`** 切换色彩主题（极品红玫、深海魅蓝、粉钻甜心、香槟流光、极光幽紫、黑珍珠等 16 种）
  - **`[Space]`** 暂停 / 继续优雅自转
  - **`[B]`** 开启 / 隐藏 3D 全息空间线框立方体
  - **`[T]`** 开启 / 隐藏 左侧原版 Python 源码虚拟滚动面板
  - **`[F / F11]`** 开启 / 退出全屏沉浸模式
  - **`[H]`** 纯净录屏模式（一键隐藏所有按钮与状态提示，适合电影感录屏）
* **超清美图截取：** 点击底部 Dock 的 **`Capture 4K`** 按钮，秒级捕获一帧 4K 极清静态图并自动下载。

### ⚡ 2. 粒子萌宠工作室操作指南
* **角色预设切换：** 点击右侧面板顶部的「⚡ 电气鼠」、「🐰 蓝色兔子」、「🐱 粉色小猫」、「🦊 紫色小狐」即可一键变身；
* **3D 视差倾斜陀螺：** 鼠标在画面中按住拖拽，角色会随指针方向产生带有平滑物理阻尼的 3D 空间倾斜视差；
* **粒子爆炸散开 (Burst)：** 在画面中轻点鼠标，或点击面板的「粒子散开一次」，数万粒子瞬间向外炸裂漫散，随后优雅聚合复原；
* **动态参数实验室：**
  - **粒子密度 (Density)：** 调节粒子采样数量，从空灵星尘到饱满毛绒感；
  - **漂移强度 (Flow)：** 微调粒子周期的呼吸微风幅度；
  - **荧光光晕 (Glow)：** 增强暗夜夜光漫反射（Bloom）光晕厚度；
  - **电弧/能量 (Energy)：** 增强皮卡丘周围随机跳跃的电弧雷光频率与长度；
* **无损透明/黑底导出：** 点击面板底部的「保存当前 PNG」即可一键保存超清电脑/手机壁纸。

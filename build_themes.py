...
<title>3D 粒子玫瑰 · 多重唯美配色与形态切换</title>
<style>
:root {
  color-scheme: dark;
  --bg: #020403;
...
.kw   { color: #ff5370; font-weight: 600; }
.fn   { color: #61afef; }
.cls  { color: #e5c07b; font-weight: 600; }
.str  { color: #98c379; }
.num  { color: #d19a66; }
...
body.quiet #ui, body.quiet #hint, body.quiet .popup-panel { opacity: 0; pointer-events: none; }
button {
...
button.highlight { background: rgba(255, 255, 255, 0.12); border: 1px solid rgba(255, 255, 255, 0.2); }

/* 配色与样式选择弹窗 */
.popup-panel {
  display: none; position: fixed; bottom: 82px; left: 50%; transform: translateX(-50%);
  padding: 18px 22px; border: 1px solid rgba(255, 255, 255, 0.12);
  border-radius: 18px; background: rgba(11, 16, 14, 0.94);
  backdrop-filter: blur(16px); -webkit-backdrop-filter: blur(16px);
  box-shadow: 0 16px 40px rgba(0, 0, 0, 0.7); z-index: 30;
}

#theme-menu { width: 330px; }
.theme-grid {
  display: grid; grid-template-columns: repeat(2, 1fr); gap: 8px; margin-top: 10px;
}
.theme-item {
  display: flex; align-items: center; gap: 8px; padding: 7px 10px;
  border-radius: 12px; cursor: pointer; background: rgba(255, 255, 255, 0.05);
  border: 1px solid rgba(255, 255, 255, 0.08); transition: all 0.2s; font-size: 12px;
}
.theme-item:hover, .theme-item.active {
  background: rgba(255, 255, 255, 0.15); border-color: rgba(255, 255, 255, 0.3); transform: translateY(-1px);
}
.theme-color-dot {
  width: 14px; height: 14px; border-radius: 50%; border: 1px solid rgba(255,255,255,0.3); flex-shrink: 0;
}

#style-menu { width: 300px; }
.style-list { display: flex; flex-direction: column; gap: 8px; margin-top: 10px; }
.style-item {
  display: flex; align-items: center; gap: 10px; padding: 9px 14px;
  border-radius: 12px; cursor: pointer; background: rgba(255, 255, 255, 0.05);
  border: 1px solid rgba(255, 255, 255, 0.08); transition: all 0.2s; font-size: 12.5px;
}
.style-item:hover, .style-item.active {
  background: rgba(255, 45, 85, 0.25); border-color: rgba(255, 45, 85, 0.5); transform: translateY(-1px);
}

#settings { width: 310px; }
...
__CODE_BODY__
  ...
  <div id="hint">
    PARTICLE ROSE · MULTI-STYLE
    <small id="status-tag">🌹 极品绯红 · 💐 华美花束 | 按 C 换色 · 按 V 换样式</small>
  </div>

  <div id="ui">
    <button id="fullscreen-btn" class="primary">⛶ 全屏</button>
    <button id="theme-btn" class="highlight">🎨 换颜色 (C)</button>
    <button id="style-btn" class="highlight">✨ 换样式 (V)</button>
    <button id="play">暂停</button>
    <button id="spin" class="active">自转</button>
    <button id="box" class="active">3D线框</button>
    <button id="code-btn" class="active">源码</button>
    <button id="options">调参</button>
    <button id="snapshot">保存壁纸</button>
  </div>

  <!-- 配色切换弹窗 -->
  <div class="popup-panel" id="theme-menu">
    <div style="font-weight:600; font-size:13px; color:#fff; display:flex; justify-content:space-between; align-items:center;">
      <span>🎨 选择花朵配色主题</span>
      <span style="font-size:11px; color:#888;">快捷键: C</span>
    </div>
    <div class="theme-grid">
      <div class="theme-item active" onclick="setTheme('ruby')">
        <div class="theme-color-dot" style="background:#ff1744;"></div> 极品绯红
      </div>
      <div class="theme-item" onclick="setTheme('blue')">
        <div class="theme-color-dot" style="background:#0096c7;"></div> 蓝色妖姬
      </div>
      <div class="theme-item" onclick="setTheme('pink')">
        <div class="theme-color-dot" style="background:#ff758f;"></div> 粉钻甜心
      </div>
      <div class="theme-item" onclick="setTheme('gold')">
        <div class="theme-color-dot" style="background:#f1c40f;"></div> 流光香槟
      </div>
      <div class="theme-item" onclick="setTheme('purple')">
        <div class="theme-color-dot" style="background:#b5179e;"></div> 极光幽紫
      </div>
      <div class="theme-item" onclick="setTheme('white')">
        <div class="theme-color-dot" style="background:#e0fbfc;"></div> 初雪冰晶
      </div>
      <div class="theme-item" onclick="setTheme('blackgold')">
        <div class="theme-color-dot" style="background:#ff6b35;"></div> 暗夜流火
      </div>
    </div>
  </div>

  <!-- 样式形态切换弹窗 -->
  <div class="popup-panel" id="style-menu">
    <div style="font-weight:600; font-size:13px; color:#fff; display:flex; justify-content:space-between; align-items:center;">
      <span>✨ 选择花朵形态样式</span>
      <span style="font-size:11px; color:#888;">快捷键: V</span>
    </div>
    <div class="style-list">
      <div class="style-item active" onclick="setStyle('bouquet')">
        <span>💐</span> <div><strong>华美花束</strong><div style="font-size:11px; color:#8b949e;">11朵层叠错落花簇 (经典视频同款)</div></div>
      </div>
      <div class="style-item" onclick="setStyle('single')">
        <span>🌹</span> <div><strong>唯美独秀</strong><div style="font-size:11px; color:#8b949e;">单朵巨型繁茂玫瑰 (近景特写拉满)</div></div>
      </div>
      <div class="style-item" onclick="setStyle('heart')">
        <span>💖</span> <div><strong>浪漫心形</strong><div style="font-size:11px; color:#8b949e;">爱心拓扑形态 (心形轮廓聚合)</div></div>
      </div>
      <div class="style-item" onclick="setStyle('cosmic')">
        <span>🌌</span> <div><strong>星河旋涡</strong><div style="font-size:11px; color:#8b949e;">宏大宇宙星轨与气旋流场</div></div>
      </div>
    </div>
  </div>

  <!-- 视觉参数弹窗 -->
  <div class="popup-panel" id="settings">
...
const THEMES = {
  ruby: {
    ...
    mid:  [1.00, 0.14, 0.18],
    rim:  [1.00, 0.94, 0.88],
    sepal:[0.26, 0.48, 0.22],
    ribbon:[1.00, 0.88, 0.90],
    box:  [0.65, 0.78, 0.92]
  },
  blue: {
    name: '蓝色妖姬',
    ...
    mid:  [0.08, 0.52, 0.98],
    rim:  [0.85, 0.96, 1.00],
    sepal:[0.12, 0.35, 0.42],
    ribbon:[0.85, 0.95, 1.00],
    box:  [0.40, 0.75, 1.00]
  },
  pink: {
    name: '粉钻甜心',
    ...
    mid:  [1.00, 0.45, 0.65],
    rim:  [1.00, 0.92, 0.96],
    sepal:[0.32, 0.62, 0.38],
    ribbon:[1.00, 0.92, 0.95],
    box:  [0.92, 0.65, 0.82]
  },
  gold: {
    name: '流光香槟',
    ...
    mid:  [1.00, 0.72, 0.18],
    rim:  [1.00, 0.97, 0.82],
    sepal:[0.36, 0.46, 0.18],
    ribbon:[1.00, 0.96, 0.88],
    box:  [0.95, 0.82, 0.45]
  },
  purple: {
    name: '极光幽紫',
    ...
    mid:  [0.72, 0.18, 0.98],
    rim:  [0.96, 0.84, 1.00],
    sepal:[0.18, 0.36, 0.38],
    ribbon:[0.92, 0.85, 1.00],
    box:  [0.82, 0.55, 1.00]
  },
  white: {
    name: '初雪冰晶',
    ...
    mid:  [0.78, 0.88, 0.98],
    rim:  [1.00, 1.00, 1.00],
    sepal:[0.35, 0.52, 0.48],
    ribbon:[0.94, 0.97, 1.00],
    box:  [0.85, 0.92, 1.00]
  },
  blackgold: {
    name: '暗夜流火',
    ...
    mid:  [0.90, 0.26, 0.05],
    rim:  [1.00, 0.85, 0.25],
    sepal:[0.18, 0.20, 0.12],
    ribbon:[1.00, 0.65, 0.15],
    box:  [1.00, 0.65, 0.20]
  }
};

let currentThemeKey = 'ruby';
let currentStyleKey = 'bouquet';

// 运行时平滑颜色渐变过渡插值器
let curColors = {
  core: [...THEMES.ruby.core],
  mid:  [...THEMES.ruby.mid],
  rim:  [...THEMES.ruby.rim],
  sepal:[...THEMES.ruby.sepal],
  ribbon:[...THEMES.ruby.ribbon],
  box:  [...THEMES.ruby.box]
};

function updateThemeColors() {
  const target = THEMES[currentThemeKey];
  const factor = 0.08;
  for (let k of ['core', 'mid', 'rim', 'sepal', 'ribbon', 'box']) {
    for (let c = 0; c < 3; c++) {
      curColors[k][c] += (target[k][c] - curColors[k][c]) * factor;
    }
  }
}
...
const ivory = [1, 0.93, 0.87], pink = [1, 0.46, 0.48], coral = [1, 0.13, 0.15];
...
// 动态花朵架构生成器
function generateSceneGeometry(style = 'bouquet') {
  ...
  if (style === 'single') {
    // 唯美独秀：1朵超大繁茂特写大花
    blooms = [{ x: 0, y: 0.25, z: 0, scale: 1.65, tilt: 0.12, az: 0, phase: 0.2 }];
  } else if (style === 'heart') {
    // 浪漫心形：中央主花 + 心形排列花簇
    blooms = [{ x: 0, y: 0.75, z: 0, scale: 0.75, tilt: 0, az: 0, phase: 0.2 }];
    for (let j = 0; j < 8; j++) {
      let t = j * TAU / 8;
      let hx = 0.85 * 16 * Math.pow(Math.sin(t), 3) / 16;
      let hz = -0.85 * (13 * Math.cos(t) - 5 * Math.cos(2*t) - 2 * Math.cos(3*t) - Math.cos(4*t)) / 16;
      blooms.push({ x: hx, y: 0.45 + 0.15 * Math.sin(t*2), z: hz, scale: 0.65, tilt: 0.68, az: Math.atan2(hx, hz), phase: rand() * TAU });
    }
  } else if (style === 'cosmic') {
    // 星河旋涡：大角度螺旋花群
    blooms = [{ x: 0, y: 0.85, z: 0, scale: 0.82, tilt: 0, az: 0, phase: 0.2 }];
    for (let j = 0; j < 9; j++) {
      let r = 0.35 + j * 0.08, ang = j * 0.72;
      blooms.push({ x: r * Math.sin(ang), y: 0.75 - j * 0.09, z: r * Math.cos(ang), scale: 0.72, tilt: 0.85, az: ang, phase: rand() * TAU });
    }
  } else {
    // 经典华美花束 (11朵盛开错落花簇)
    blooms = [{ x: 0, y: 1.05, z: 0, scale: 0.78, tilt: 0, az: 0, phase: 0.2 }];
    for (let j = 0; j < 4; j++) {
      let a = j * TAU / 4 + 0.35;
  ...
  for (const b of blooms) {
    const layerMax = style === 'single' ? 5 : 5;
    const baseCount = style === 'single' ? 750 : 350;
    for (let layer = 0; layer < layerMax; layer++) {
      for (let j = 0; j < petalCounts[layer]; j++) {
        const count = baseCount + layer * 28;
        for (let k = 0; k < count; k++) {
          ...
          vertex(particles, p, c, a, 0.84 + rand() * 0.57, isMist ? 1.55 : 0.54, rand() * 100);
        ...
        for (let h = 0; h < 7; h++) {
          ...
          for (let step = 0; step < 24; step++) {
            ...
            if (prev) {
              vertex(threads, prev, [1, 0.84, 0.77], 0.115, 1, 1.15, seed);
              vertex(threads, p,    [1, 0.84, 0.77], 0.115, 1, 1.15, seed);
            }
            prev = p;
          }
        }
        for (let h = 0; h < 13; h++) {
          ...
          for (let s = 0; s < 14; s++) {
            ...
            if (prev) {
              vertex(threads, prev, col, 0.105, 1, 0.85, seed);
              vertex(threads, p,    col, 0.105, 1, 0.85, seed);
            }
            prev = p;
          }
        }
    ...
    for (let k = 0; k < 1000; k++) {
      ...
      vertex(particles, p, lerpc(coral, ivory, rand() * 0.45), 0.34, 1, 1.1, 0);
    }
  }
  ...
  for (let j = 0; j < 15; j++) {
    ...
    for (let k = 0; k < 2400; k++) {
      ...
      let p = [r * Math.sin(ang), y, r * Math.cos(ang)].map(x => x + gaussian() * 0.026);
      let c = lerpc([0.24, 0.27, 0.13], [0.79, 0.76, 0.55], smooth(0.25, 1, v));
      c = lerpc(c, [0.88, 0.79, 0.65], smooth(0.75, 1, Math.abs(u)) * 0.5);
      vertex(particles, p, c, 0.13, 0.95 + rand() * 0.6, 1.1, seed + rand());
    }
  }
  ...
  for (let k = 0; k < 42000; k++) {
    ...
    vertex(particles, p, c, 0.24, 1.15 + rand() * 0.40, 1.5, 0);
  ...
  for (let j = 0; j < 25; j++) {
    ...
    for (let k = 0; k < 520; k++) {
      ...
      vertex(particles, p, c, 0.32, 0.90 + rand() * 0.45, 1.4, seed + rand() * 3);
    }
  }
  ...
  for (let side of [-1, 1]) {
    for (let k = 0; k < 7800; k++) {
      ...
      let p = ribbonPoint(side, t, w).map(x => x + gaussian() * 0.009);
      let c = lerpc([0.97, 0.88, 0.82], [1, 0.66, 0.72], 0.18 + 0.14 * Math.sin(t * TAU));
      vertex(particles, p, c, 0.41, 1.05, 1.4, 0);
    }
    for (let j = 0; j < 45; j++) {
      ...
      for (let s = 0; s < 60; s++) {
        ...
        if (prev) {
          vertex(threads, prev, [0.93, 0.80, 0.77], 0.065, 1, 1.5, seed);
          vertex(threads, p,    [0.93, 0.80, 0.77], 0.065, 1, 1.5, seed);
        }
        prev = p;
      }
    }
    for (let k = 0; k < 1400; k++) {
      ...
      vertex(particles, p, [0.79, 0.62, 0.62], 0.25, 1, 1.5, 0);
    }
  }
  ...
  for (let j = 0; j < 4400; j++) {
    ...
    for (let k = 0; k < 18; k++) {
      ...
      if (prev) {
        vertex(threads, prev, c, (0.048 + 0.026 * (1 - t)) * (1 - t * 0.6), 1, 1.8, seed);
        vertex(threads, p,    c, (0.048 + 0.026 * (1 - t)) * (1 - t * 0.6), 1, 1.8, seed);
      }
      if (k % 2 === 0) vertex(particles, p, c, 0.15 * (1 - t * 0.65), 1.05, 1.8, seed);
      prev = p;
    }
  }
  ...
  for (let k = 0; k < 7000; k++) {
    ...
    vertex(particles, p, lerpc(a.c, ivory, 0.35), 0.11, 1.35 + rand() * 0.85, 2.6, 0);
  }
...
uniform int uBox;
uniform vec3 uThemeCore, uThemeMid, uThemeRim, uThemeSepal, uThemeRibbon, uThemeBox;
...
vec3 getThemeColor(vec3 base) {
  // 萼片特征: 绿色通道显著偏高
  if (base.g > base.r * 1.05 && base.g > 0.15) {
    float lum = dot(base, vec3(0.299, 0.587, 0.114));
    return mix(uThemeSepal * 0.5, uThemeSepal * 1.35, smoothstep(0.1, 0.65, lum));
  }
  // 丝带特征: 高亮且各通道相近
  float minC = min(base.r, min(base.g, base.b));
  float maxC = max(base.r, max(base.g, base.b));
  if (minC > 0.55 && (maxC - minC) < 0.28) {
    float lum = dot(base, vec3(0.333));
    return mix(uThemeRibbon * 0.85, vec3(1.0, 0.98, 0.95), smoothstep(0.7, 1.0, lum));
  }
  // 花茎特征
  if (base.r > base.g * 1.4 && base.r < 0.62 && base.b < 0.22) {
    return mix(uThemeCore * 0.7, uThemeCore * 1.2, base.r);
  }
  // 花瓣主体与高光卷边
  float lum = dot(base, vec3(0.299, 0.587, 0.114));
  float rim = smoothstep(0.64, 0.93, max(base.b * 1.6, lum));
  vec3 col = mix(uThemeCore, uThemeMid, smoothstep(0.10, 0.56, lum));
  col = mix(col, uThemeRim, rim);
  return col;
}
...
void main() {
  ...
  vec3 baseColor = uBox == 1 ? uThemeBox : getThemeColor(aColour);
  vColour = baseColor * (uBox == 1 ? 1.0 : light);
...
function upload(vertices, existingVao = null, existingBuffer = null) {
  const vao = existingVao || gl.createVertexArray();
  ...
  return { vao, buffer, count: vertices.length / 10 };
}

let sceneGeo = generateSceneGeometry('bouquet');
let pointGeometry = upload(sceneGeo.particles);
let threadGeometry = upload(sceneGeo.threads);
...
function geometry(p, g, mode, t, isBox) {
  ...
  // 传递平滑渐变主题颜色 Uniforms
  gl.uniform3fv(loc(p, 'uThemeCore'), curColors.core);
  gl.uniform3fv(loc(p, 'uThemeMid'), curColors.mid);
  gl.uniform3fv(loc(p, 'uThemeRim'), curColors.rim);
  gl.uniform3fv(loc(p, 'uThemeSepal'), curColors.sepal);
  gl.uniform3fv(loc(p, 'uThemeRibbon'), curColors.ribbon);
  gl.uniform3fv(loc(p, 'uThemeBox'), curColors.box);
...
function updateStatusTag() {
  const themeName = THEMES[currentThemeKey].name;
  const styleNames = { bouquet: '💐 华美花束', single: '🌹 唯美独秀', heart: '💖 浪漫心形', cosmic: '🌌 星河旋涡' };
  document.getElementById('status-tag').textContent = `${themeName} · ${styleNames[currentStyleKey]} | 按 C 换色 · 按 V 换样式`;
}
...
window.setStyle = function(styleKey) {
  ...
  updateStyleActiveUI(styleKey);
  updateStatusTag();

  // 动态重新生成几何数据并上传至 GPU
  const newGeo = generateSceneGeometry(styleKey);
  upload(newGeo.particles, pointGeometry.vao, pointGeometry.buffer);
  pointGeometry.count = newGeo.particles.length / 10;
  upload(newGeo.threads, threadGeometry.vao, threadGeometry.buffer);
  threadGeometry.count = newGeo.threads.length / 10;
};
...
function cycleTheme() {
  ...
  currentThemeKey = keys[nextIdx];
  updateStatusTag();
}

// 快捷键循环换样式
function cycleStyle() {
  const styles = ['bouquet', 'single', 'heart', 'cosmic'];
  const nextIdx = (styles.indexOf(currentStyleKey) + 1) % styles.length;
  window.setStyle(styles[nextIdx]);
}
...
$('snapshot').onclick = () => {
  ...
  canvas.toBlob(blob => {
    ...
    a.download = `particle-rose-${currentThemeKey}-${currentStyleKey}.png`;
    a.click();
...
document.addEventListener('pointermove', () => {
  ...
  clearTimeout(idle);
  idle = setTimeout(() => {
    if ($('settings').style.display !== 'block' && $('theme-menu').style.display !== 'block' && $('style-menu').style.display !== 'block') {
      document.body.classList.add('quiet');
    }
  }, 5000);
});
idle = setTimeout(() => document.body.classList.add('quiet'), 5500);
</script>
</body>
</html>"""

full_html = template.replace('__CODE_BODY__', code_html)
for target in ['particle_rose_fullscreen.html', 'index.html', 'particle_rose_diffusion.html']:
    with open(target, 'w', encoding='utf-8') as f:
        f.write(full_html)
    print(f'Updated {target}, size: {len(full_html)}')
